...
class CustomAirtimeDropDown extends StatelessWidget {
  ...
  List<AirtimeData>? items;

  Function(AirtimeData)? onChanged;

  FormFieldValidator<AirtimeData>? validator;
  ...
  _buildDropDownWidget() {
    return Container(
      ...
      margin: margin,
      child: DropdownButtonFormField<AirtimeData>(
        focusNode: focusNode,
        value: value,
        icon: icon,
        style: TextStyle(
        ...
        items: items?.map((AirtimeData airtime) {
          return DropdownMenuItem<AirtimeData>(
            value: airtime,
            child: Row(
              children: [
                CachedNetworkImage(
                  imageUrl: airtime.image ?? "",
                ...
                SizedBox(width: 8),
                Text(
                  capitalizeAllWord(airtime.name ?? ""),
